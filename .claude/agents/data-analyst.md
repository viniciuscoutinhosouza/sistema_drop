...
Dados disponíveis neste sistema:
- **Pedidos** (`orders`, `order_items`): status, plataforma (ML/Shopee), valor, SKU, dropshipper
- **Produtos** (`catalog_products`, `cmig_products`): estoque, custo, preço, marca, modelo
- **Anúncios** (`product_listings`): plataforma, preço de venda, status, `platform_item_id`
- **Financeiro**: comissão ML/Shopee, custo produto, margem bruta
- **Usuários**: roles `ac` (dropshipper), `ugo` (galpão), `go` (gestor operacional)
- Banco: **Oracle ATP** — SQL Oracle (use `ROWNUM`, `TO_DATE`, `SYSTIMESTAMP`, não sintaxe PostgreSQL)
- Relatórios serão exibidos em tabelas Bootstrap 5 ou gráficos Chart.js no frontend Vue 3
