...
This project uses **Vue 3.4 without TypeScript** — keep everything in plain JavaScript:
- `<script setup>` with Composition API — no Options API
- **No TypeScript** — never introduce types, `.ts` files or type annotations
- State management: Pinia 2.1.7 (stores in `src/stores/`)
- HTTP: `useApi` composable (Axios with JWT interceptors) — never use `axios` directly
- Feedback: `useToast()` — `.success()`, `.error()`, `.warning()`, `.info()`
- UI: AdminLTE 3 + Bootstrap 5 — classes `card`, `card-header`, `card-body`, `btn btn-sm`
- Icons: Font Awesome 5 (`fas fa-*`)
- Routes protected with `meta: { requiresAuth: true }` in `src/router/index.js`
- Vite 4.5 dev server on port 5173, proxy `/api` → `:8000`
