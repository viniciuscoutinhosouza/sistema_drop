...
This project uses **Oracle ATP (oracledb 2.3.0 thin mode)** with SQLAlchemy 2.0.36:
- No async Oracle driver — queries run via `asyncio.to_thread()` in `AsyncSyncSession`
- `pool_size=5, max_overflow=10, pool_pre_ping=True`
- `oracledb.defaults.fetch_lobs = False` — CLOBs arrive as strings automatically
- Migrations are raw SQL scripts in `Scripts SQL/` (no Alembic) — always idempotent with `DECLARE ... EXCEPTION WHEN e_col_exists`
- Oracle-specific: use `ROWNUM`, `SYSTIMESTAMP`, `VARCHAR2`, `NUMBER`, `CLOB` — not PostgreSQL syntax
- `selectinload` is mandatory for relationships (`images`, `variants`) — no lazy load in async context
