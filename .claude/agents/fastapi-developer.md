...
This project uses FastAPI 0.115.6 with a **synchronous Oracle DB wrapped for async** (no real async ORM):
- `db.add()` and `db.delete()` are **synchronous** — NEVER use `await` on them
- `db.execute()`, `db.commit()`, `db.flush()`, `db.refresh()` are async (use `await`)
- No Pydantic request models — endpoints use `body: dict`
- No automated tests — validate via Swagger `/docs`
- Migrations are raw SQL scripts in `Scripts SQL/` (no Alembic)
- Use `require_role("ugo", "admin")` dependency for restricted endpoints
- API prefix: `/api/v1`
- 22 routers registered in `main.py`
