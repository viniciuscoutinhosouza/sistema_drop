...
Pré-avaliado (consistency-auditor: pegou SVG/XSS, autorização, limite real) + auditado (quality-guardian + consistency-auditor): **sem CRITICAL/HIGH**. Commit `cd63a3e`. **Deployado + verificado ao vivo** (2026-09-24): migração 141 aplicada (6/6 colunas), backend startup limpo /docs 200, frontend rebuild OK. Provado contra o banco real: `_serialize` expõe os 6 campos; update salva cores (normaliza `#1e3a8a`); **cor inválida `red; }body{` → 400** (CSS injection bloqueado); **SVG/XSS rejeitado** por magic bytes (`_sniff_image_ext`→None); rotas `POST/GET /warehouse/{id}/logo` no ar e gated (401 sem token). O upload real do arquivo + a aplicação visual do tema serão exercitados pelo dono na UI.
...
**2ª parte (mesma sessão) — extrato de movimentação:** o dono notou que a 2371 não aparecia no extrato. Causa: as 137 remessas às filiais não cadastradas foram capturadas como `purpose='venda'`, `stock_updated=0`, `move='none'` (pelo mesmo bug de `is_full_cnpj` na captura) → não debitavam o galpão nem geravam trilha FULL (`StockMovement field_affected='full_stock'`), só o saldo FULL (via recompute). **Decisão do dono:** a remessa **debita o galpão (−qtd) e credita o FULL (+qtd)** — aparece nas duas seções (as mercadorias já tiveram entrada). **Backfill Option B aplicado (dados, cmig 1):** as 137 remessas EBAZAR não-simbólicas → `purpose='remessa'`, `stock_updated=1` (debita galpão via replay local) + `apply_nfe_saida_to_full` (cria a trilha FULL; idempotente) + `recompute_full_stock` (saldo absoluto). 2371 agora com `stock_updated=1` + 7 StockMovements FULL; galpão PG 169 234→72. **Pendência (dono vai avaliar o extrato e instruir):** 21 produtos ficaram com galpão **negativo** (déficit remessa>entrada — falta a entrada/baseline do estoque importado de conta nova). Dono **pediu para NÃO usar baseline** por ora, deixar só as entradas/saídas reais para ele analisar o extrato e depois instruir a correção. Lista dos 21 negativos (PG/CMIG) entregue. **Nenhuma alteração de código nesta 2ª parte — só dados.**
...
**Auditoria prévia:** consistency-auditor (incorporados: order_sn ambas chaves como fix de perda de pedido; não cristalizar pedido pobre; token+detalhe no try; aviso de moeda). **Auditoria de fechamento (quality-guardian) — CRITICAL corrigido:** o `return` após falha do detalhe fazia o chamador marcar `event.processed=True` → pedido nunca retentado (perda). Corrigido: a falha do detalhe agora **propaga** (removido o `except` amplo) — no sync o `except` faz rollback (o WebhookEvent usa flush, não commit) e retenta; no push vira 5xx e a Shopee reenvia; também é "falhar alto" (bug de parsing propaga em vez de sumir). LGPD: log de evento malformado passa a logar só as chaves (não o payload com PII). ML intocado (confirmado byte-idêntico). **Follow-ups:** re-hidratar pedido já criado porém pobre; busca de detalhe em lote no sync; validação estrita de moeda; `buyer_document` → Fase 3. **Falta:** smoke com pedido de teste real no sandbox (loja vazia).
...
**Push/webhook (2026-07-22):** a tela "Definir Push" do console verifica o webhook mandando um test push que exige resposta **2xx**. Dois fixes: (1) a Shopee assina o push com a **Push Partner Key** (separada da de API) → novo `SHOPEE_PUSH_PARTNER_KEY` (commit 8bdd476); (2) o nginx não repassa `X-Forwarded-Proto`, então o webhook reconstruía a URL como `http://` e a assinatura (`https://`) não batia → agora tenta **ambos os schemes** contra a push key e a de API (commit 771550a). Verificado ao vivo: test push assinado (https) → **200**; inválido → 401. **URL do webhook:** `https://ecommerce.madeingroup.com.br/api/v1/webhooks/shopee`. Push key de teste no `.env` (`SHOPEE_PUSH_PARTNER_KEY`).

**⛔ BLOQUEIO ATUAL (lado da Shopee, não do sistema) — ponto de retomada:** ao autorizar a loja de teste, o **callback nunca chega** ao nosso servidor (verificado em todos os logs nginx do IP do dono). O DevTools do dono mostrou o motivo: **`POST account.sandbox.test-stable.shopee.com/api/account/login/ → 403 Forbidden`** — o **login do sandbox está sendo recusado** na tela direta do OAuth. Bate com o aviso do console ("Seller Center Sandbox só acessível via botão de login, não por URL direta"). **Do nosso lado está 100% pronto** (host sandbox v2, assinatura, redirect com state no caminho, callback esperando). Retomada: (1) logar no sandbox pelo botão **"Login Seller Center"** (console → Test Account-Sandbox, shop 227767957) numa janela NORMAL e, na MESMA sessão, fazer "Conectar Shopee"; (2) se persistir o 403, abrir **Raise Ticket** na Shopee. Quando a loja autorizar, o callback dispara `exchange_code`+`get_shop_info` e conecta → seguir para o **pré-voo** (Get* read-only) e Fases 2→7 do `DOCs/PLANO_PARIDADE_SHOPEE.md`. Credenciais Test só nos `.env` (partner_id 1238914, key `shpk786e…`, api_base `openplatform.sandbox.test-stable.shopee.sg/api/v2`, redirect `/accounts/shopee/callback`).
