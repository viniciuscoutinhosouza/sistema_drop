from sqlalchemy import Column, Integer, String, Boolean, TIMESTAMP, ForeignKey, text as sa_text, Text, UniqueConstraint
...
class ConversationThread(Base):
    ...
    status               = Column(String(30), nullable=False, default="open")  # open | pending_reply | closed | answered
    ...
    updated_at           = Column(TIMESTAMP(timezone=True), server_default=sa_text("SYSTIMESTAMP"),
                                  onupdate=sa_text("SYSTIMESTAMP"))

    __table_args__ = (
        UniqueConstraint("platform", "platform_thread_id", "marketplace_account_id",
                         name="uq_thread_platform_id"),
    )

    account  = relationship("MarketplaceAccount")
    messages = relationship("ConversationMessage", back_populates="thread",
                            order_by="ConversationMessage.sent_at", cascade="all, delete-orphan")
...
class ConversationMessage(Base):
    ...
    ai_suggested_text   = Column("ai_suggested_text", Text)  # audit: o que a IA sugeriu antes do envio
