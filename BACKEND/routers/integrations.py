...
async def _import_shopee_listings(account, user, db) -> dict:
    """Importa os anúncios ATIVOS da loja Shopee (get_item_list → get_items_base_info) como
    DropshipperProduct + ProductListing. Dedup por (account, platform_item_id). Ramo Shopee."""
...
async def import_listings(
    ...
    """Importa todos os anúncios ativos desta conta do marketplace (ML ou Shopee)."""
    account = await _assert_ac_can_access(account_id, current_user.id, db)
    if account.platform == "shopee":
        return await _import_shopee_listings(account, current_user, db)
    if account.platform != "mercadolivre":
        raise HTTPException(
            status_code=400, detail="Importação de anúncios não disponível para esta plataforma."
        )
