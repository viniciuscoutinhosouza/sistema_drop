...
- **Fase atual:** Emissão própria de NF-e direto à SEFAZ, substituindo o Focus (ADR-0015/0016).
- **Último ponto validado:** Emissão própria SEFAZ implementada (2026-06-29) — Focus **removido por completo** (backend + frontend). Camada pura `services/fiscal/sefaz/` (chave/xml_builder/signer XMLDSig/sefaz_client mTLS/emitter/consulta/cancelamento+CC-e/manifestação/inutilização/distribuição DFe/DANFE) portada do projeto-fonte (cStat=100) e adaptada; `sefaz_service` liga aos models existentes (CMIG/Person/CMIGProduct/Invoice). Emissão manual em **série configurável separada** do Faturador ML (preservado). Entrada via **Distribuição de DFe própria** (NSU por CMIG). Certificado A1 local + senha cifrada (Fernet/`NFE_CERT_MASTER_KEY`); XML autorizado fora de `static/`. Correções fiscais do Consultor-Fiscal-NFE (CSOSN 102/500, PIS/COFINS CST 99, sem IPI, sem DIFAL-partilha p/ Simples). Migrations 115-118. Auditado por quality-guardian + consistency-auditor + adr-consistency-checker — CRITICAL (XML em `static/`) e HIGH (parser/zip-bomb, tz ADR-0013, paridade de download) **corrigidos**. `pytest -m "not integration"` **78 passed / 2 falhas pré-existentes**; `npm run build` OK; DANFE PDF e parser DFe smoke-tested.
- **Próximo passo:** **Smoke em HOMOLOGAÇÃO** (precisa cert A1 real + `NFE_CERT_MASTER_KEY` + cadastro CMIG completo: IBGE/IE/série manual) — emitir NFe-55 RJ(SVRS) e SP → cStat=100; validar Distribuição DFe e endpoints AN (`SEFAZ_DFE_AN`/`SEFAZ_EVENTO_AN`, `distDFeInt` versao 1.01 — ajustáveis). Depois: produção faseada por CMIG (`production_released=1`) após credenciamento + cabundle ICP-Brasil. Migrations 115-118 já rodadas pelo dono.
