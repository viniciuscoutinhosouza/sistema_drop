...
onMounted(async () => {
  ...
  if (isEdit.value) {
    ...
    // Pré-popula categoria vinda do modal de pré-validação no Catálogo
    const qsCatId   = route.query.category_id
    const qsCatName = route.query.category_name
    if (qsCatId) {
      form.category_id = String(qsCatId)
      form.category_name = qsCatName ? String(qsCatName) : ''
      await Promise.all([
